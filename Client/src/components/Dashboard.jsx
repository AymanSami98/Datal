...
function Dashboard() {
  ...
  const durations = chartData.map((item) => parseFloat(item.duration));
  const monthlyDates = monthlyChartData.map((item) => item.date);
  const monthlyDurations = monthlyChartData.map((item) =>
  ...
  const reportssessionsCountData = reports.map(
    (report) => report.sessionsCount
  );
  ...
  return (
    ...
    />
              ...
              series={[
                {
                  ...
                  label: "Monthly Plays Time (minutes)",
                },
              ]}
      ...
      <BarChart
        series={[{ data: monthlyPlaysDates, label: "Monthly Plays Number" }]}
        width={1200}
        height={300}
        xAxis={[{ scaleType: "band", data: monthlyDates }]}
        sx={{
